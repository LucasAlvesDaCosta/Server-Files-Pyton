...
def send_list( con):
	save_list()
	lista = open(make_folder_tmp_srv('lista_de_arquivos.s'),'rb',buffering = -1,encoding = None)
...
def make_zip(name):
	...
	zip = zipfile.ZipFile(make_folder_tmp_srv('zip_master.zip'), 'w', zipfile.ZIP_DEFLATED)
